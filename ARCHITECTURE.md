...
running it. The Go side passes it only when `COEUS_HEADLESS_TESTS` is set, which
...
**No window during a test run.** The integration tests start a real Chrome, and a real Chrome puts a window on the screen of whoever is running them. So `make test-browser` sets `COEUS_HEADLESS_TESTS`, and `workerArguments` adds `--headless` to the worker's command line when that name is set and nothing when it is not; the worker turns it into Chrome's own `--headless=new` and `--disable-gpu`. `TestChromeIsAskedToGoHeadlessOnlyWhenTheTestsAskForIt` holds both halves, and the TypeScript half is held by `worker/browser/test/headless.test.ts`. Nothing else sets that name, so an ordinary run still opens the visible window design section 11 asks for, because a logged-in account is only safe in a window the user can see. `test/functional/browserworker_integration_test.go` starts the same worker by hand and keeps the same two rules: the headless flag when the name is set, and an environment with no session message bus in it.
...
`~/coeus`, which `coeus init` creates. `checkSandboxRoots` hands the check this
...
`coeus init` passes neither, and says why: it is the thing that writes the first
...
**The folder format.** One folder per skill under `~/.coeus/skills`, holding four files. `SKILL.md` is a heading with the skill's name, one line of description under it, a `## Triggers` section of bullets, and a `## Permissions` section of `- key: value` bullets, where the four keys are `browser profile`, `site`, `daily limit`, and `irreversible step`; a fifth key is a typo that would quietly widen what the skill may do, so it is refused by name. A `site` value is one bare host name and nothing else: letters, digits, dots and hyphens, at most 253 bytes, with every part between its dots at most 63 and neither beginning nor ending in a hyphen. No star, no question mark, no scheme, no path, no port, no space. Anything else is refused where the line is read, with the line quoted back, and refused again where the standing approval is built, because that is where a site line turns into permission to act. `steps.md` is a numbered list where each step's first line is what it is for and the indented lines under it are `tool:`, `input:` and `expect:`; a line that is none of those three is more of the step's own words, so a step can explain itself over several lines. A step with no `tool:` is a step written in words, which replay cannot run and hands back to the model. `input:` is the JSON the tool is given, with `{{arguments}}` standing where the run's own arguments belong, escaped so the input is still JSON whatever the user typed. In place of `steps.md` a skill may carry an executable named `script`, which replays as one shell step. `test.md` is the dry run: an `arguments:` line and an optional `expect:` line, and everything else in it is prose. `CHANGELOG.md` records every change with a way to undo it, and the copy each save replaced sits under `versions/<n>/`. The numbers on a step list must run one, two, three, because a list whose numbers jump has had something cut out of it and replaying half a procedure is worse than replaying none.

**Forgiving to list, strict to use.** `List` and `Match` read only `SKILL.md` and pass over any folder they cannot read, because the listing rides in every prompt and one broken folder must not empty it. `Load`, `Run` and `DryRun` read the whole folder and refuse it with the missing file named, because using a broken skill must fail loudly. A folder's name and the name in its heading have to agree. Save completes what it is given: a caller who wrote only a `SKILL.md` gets an empty `steps.md`, a default `test.md`, and the changelog already on disk with a new line on the end, so everything the store saves can afterwards be loaded and run. Every bound is a named constant in `format.go`: two hundred skills, sixty-four characters of name, two hundred of description, twenty triggers, twenty websites, fifty steps, four thousand bytes a step, a quarter of a megabyte a file, twenty kept versions, and a daily limit between one and a thousand; the two bounds on a website's host name, 253 bytes and 63 for one part of it, are in `site.go` beside the rule that uses them.

**The permissions block, enforced.** When a skill runs, each website its block names becomes `permission.StandingApproval` values, one for every readable form a visit to that website takes: the name of a tool that visits a website, a space, `https://` or `http://`, the host, and then either the end of the form or the slash that starts a path. The tool's name and the scheme are written out in full, so nothing in front of the host is left open and a shell command that merely carries a website's name inside it never matches; the host is followed by the end of the form or by a path, so `news.example.com` covers `news.example.com/today` and covers neither `notnews.example.com` nor `news.example.com.evil.net`. The tools are the ones the skill's own steps name, out of the four whose call carries a web address — `web`, `browser_open`, `browser_read`, `browser_login` — because a replay makes no call its steps do not name; a skill that carries a script, or whose steps are written in words, registers nothing at all. Each form carries the block's daily limit and the next midnight as its expiry, so a site's budget is counted per form. They are registered once a day per skill, so running a skill twice does not hand it twice its budget. Two things fall outside on purpose, both of them fail-closed: an address that names a port is not covered, because `<host>:` in a pattern would also cover `<host>:anything@somewhere.else`, and neither is a subdomain of a named site, which the block's own gate still lets a step visit but which the permission function will ask about if a rule covers the call. And there are now four forms per website per tool where there used to be one pattern, against a session's hundred standing approvals, so a skill naming many websites and visiting them with several tools can run the session's approvals out; the permission function refuses the extra one by name, and the lasting fix is for a standing approval to carry the tool and the host it covers rather than a pattern over the readable form. This is the fix for the first finding of the wave 6 security review: the form used to be `*<site>*`, and a site line of `*` made it `***`, which matched the readable form of every call there is, so a page the agent read could have written the agent a permission slip for everything. Then every step goes through three gates in order: a browser or web step whose address the block does not name is put to the user; the permission function rules on the call, and a deny stops the skill while an ask goes to the user; and a step the block marks as one that cannot be undone is previewed even when it was allowed. All three ask through one function value, `skill.AskFunc`, whose shape is exactly `contract.Channel.ShowPreview`, so `cmd/coeus/serve.go` wires the user's channel straight into it; a refusal carries the user's own words into the error, so the model is told why. With nothing wired, a step that needs a yes stops and says what it needed, which is what an unattended run gets.
