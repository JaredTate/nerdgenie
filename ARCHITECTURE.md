...
**Records are checkpoints in the log.** `internal/record` prints a task record or a job record as text in the four parts of an operations order (goal, rules, work, lessons) and saves every change as a numbered checkpoint event. A task that waits for days holds nothing in memory; it is rebuilt from its last checkpoint when the person answers, on any model. `/tasks 17 back 3` winds a task back three checkpoints. A lesson is written once: a failure whose text, or whose cause, says what a held failure already says, four words in five shared, is refused naming the failure that says it (`sameness.go`), because the sky task of the flight-simulator work order wrote one cause as four failures in four wordings and both its rethinks named it again (8 September 2026).
...
**Every call is guarded, permitted, and run.** The same-call guard refuses a call the model repeats word for word inside its window (`guard.go`: `IdenticalCallsAllowed` 2, `SameCallHardCap` 6). A repeat with the same answer counts only when what came back between it and the last repeat matches what came back the time before, or nothing came between; a repeat after new answers is work, such as New Match clicked between games (8 September 2026). The permission function decides allow, ask or deny (section 11). The tool runs through the registry, its result gets one line in the record and its whole text in the log, and text past the output cap goes to a spill file the model can read (`calls.go`, `recordline.go`).

**The harness checks a change itself.** After every write or edit the file is parsed with the language's own checker (`syntax.go`), and once the model has run the tests, they are run again after each change with the answer on the change's own result (`autotest.go`, the readers in `teststate*.go`). An `expect` line on a shell, write or edit call is judged by four rules, a test count, an exit code, a contained string, parses (`expect.go`). A write to a code file that no failing test covers gets one line saying so, and a write or an edit to any file made once every done line of the task is proved, with the newest test run green or none, gets one line saying the edit serves no line and to finish or name the line it serves (`testsfirst.go`). A shell command still running after ten seconds returns an id to poll (`commands.go`, `probes.go`).

### The guards

The loop counts what moved and acts on a stretch in which nothing did (`progress.go`). A round counts as progress when a test run shows fewer failing tests than any run before it in the task (a count seen before is more of the same, since run 28's minimax task flipped between two and three failing for forty minutes), a step or a done line is marked, a page changes under an action, a new file is written, or something new is read. At `NudgeAfterRoundsWithoutProgress` (10) the model reads one line asking for a diagnosis. At `RewindAfterRoundsWithoutProgress` (20) the rounds since the last progress are cut from the conversation and the rest kept, the stall goes into the record as a failure, and the model gets a rethink: one call with the tools off asking what it was trying to learn and which call would tell two causes apart, with the call it kept repeating closed for `ClosedForRounds` (10) rounds (`rethink.go`, `closedcalls.go`, `guard.go`'s `rewindIfDue`). The rethink's two causes are held against every failure's cause on the record by the record's own rule, and when both are already there the question is asked once more, once, with a line in front of it naming the failures that say so, and the second answer is taken as it stands; a task picked up after a stop whose record holds a failure opens with the same rethink before its first working call, with the person's or the job's word after it, because the sky task was picked up three times and each time went straight back to the failure list that had stopped it (`rethink.go`, `freshwindow.go`, 8 September 2026). Three such cuts are allowed; the fourth stall stops the task (`StallsBeforeStop` 4). Between the meter and the guard sits the same-wall detector (`samewall.go`): it buys the same rethink when one call has come back with the same result `SameWallRecurrences` (6) times inside its window, whatever the calls between and whatever they returned, which is the case both other nets read as work — the guard starts its count again the moment a call in between answers something new, and the meter counts a new file or a new read as progress, so a model running a frozen check between edits that each write a new file is invisible to both until the meter's slow twenty rounds; the tic-tac-toe-against-the-computer task ran ninety rounds that way on 8 September 2026. It reads a call-and-result pair, not a bare result, so an unchanging confirmation from a changing call is not mistaken for a wall, and it leaves the volatile tokens of a result — a clock time, a process id, a port, a path — out of the pair, but not a duration, which the test-state tracker already reads past. Three such cuts share the meter's budget. A task that has made `CallsBeforeTheFinishNudge` (100) calls reads one line, once, whatever the meter made of its rounds: finish if the done lines are met, or write what blocks it as a failure and finish; run 23's visual QA task polished one SVG ring for 157 calls with every round counting as progress. A task the guard stopped and its job picked up is told why in the guard's own words and asked to finish what is provable rather than continue (`jobs.go`; 8 September 2026). The message cap opens a fresh window instead of dropping the oldest half: the orientation with the newest six results, one line saying what happened, and the ask last (`freshwindow.go`). A reply cut off at the output cap is told so and asked to write a long file in parts (`cutoff.go`). A hung tool is killed after the tool's time limit, and a page that hangs the browser is reported with the loop that never yields (`hangs.go`).

### The done check and the review

A task cannot close on the model's word. `donecheck.go` reads the done list: every line must point at a result or a reply from the person; a line that names a file or a command is checked by the harness, and a line that points at a test run older than the last edit is sent back once. A line that ends with a bracketed check, `[tests pass: npm test]`, `[exit 0: ...]`, `[shows: "text" at url]`, `[exists: path]` or `[looks: url at 1440, 768, 390]`, is run by the harness itself (`linecheck.go`) and is never judged by the model; the looks check opens the page at each width through the browser tools, photographs it, asks it how far it overflows the window and reads its page errors, and refuses with the width and the fact, "at 390 the page overflows by 42 pixels", so the model fixes that and nothing else (`lookscheck.go`, 8 September 2026). A done line that says photographed or screenshot is proved by the harness's own picture, never the model's: it resizes the page to each width the line names, "at 1440" or "390 wide", at most two, photographs it through the model's own browser tools, reads off the screenshot tool's line how many frames the page drew in a quarter of a second, and pins the line to the picture's result when the page drew any; a page that drew none proves nothing, and the model is told to make the page draw and then close; with no browser tool wired the line stays the model's to prove (`photograph.go`, 8 September 2026). A task with every done line proved that reports a stop is a finish (`stopdone.go`). When a job's checked line fails at a task's end, the task's progress line names the line and the check's own words, bounded, so the next task knows what to put right (8 September 2026). A page check that meets the browser's restart message, because the idle worker died during a task that never used it, opens the page once more before it says the line is not true. A page whose server is down is reported by the worker as unreachable (-32004) with the browser left alone, since run 26 showed the worker calling a refused connection a dead Chrome and restarting the browser twice for a page that was simply not there. Then, when the task had a correction, a failure, a stop or more than five rounds, the review asks four questions with the tools off, saying so in the question, and keeps the last answer as a fact or offers it as a skill (`review.go`), and asks a fifth when the folder has an architecture page (section 6, `archpage.go`). Every question asked with the tools off goes into the log as a `question` event with its answer and what came of it: the review, the section question, and the rethink a stall buys (`rethink.go`, 8 September 2026), so a page that was not written can be traced to the answer that named no section, and a stall to what the model thought it showed.

## 6. Jobs and work orders

A task is one sitting of work. A job is work too big for one sitting: a record of the same four parts whose plan is a list of tasks and whose results are those tasks' reports (`internal/job`). The harness runs one task at a time, writes each report into the job and sends it to the person, and starts the next. A task's done list holds at most five lines and its plan at most ten steps; a sixth line or an eleventh step is refused with the words "this ask is a job", and the model makes one with the `job` tool. A job with a schedule makes its tasks from the clock. A task a guard stopped inside a job is picked up once by the job itself, on a fresh window (`putdown.go`, `jobs.go`); a second such stop sets the task aside instead of waiting for a person, through `contract.Job.Defer`, which answers yes once per task and lets the task's claim go: the person reads one line, the task's record is kept as a put-down task's is, the store passes the task over while any other task ahead of the job's last is unfinished and not deferred, and takes it again, afresh on its own words, once only deferred tasks remain there, in their order, before the last task, which waits for it; a third stop puts the task down for the person as before (8 September 2026, after the flight simulator's sky task waited nine hours for a person three times while twelve tasks that needed nothing from it sat untouched). Under yolo the agent is unattended, so a job task that would be put down for a person — a third guard stop, a paced-out comeback, or a question — is instead handed back to the model on a fresh window with a bold reorient ask that names its own failures, tells it to drop what it already tried, look with completely fresh eyes, and take a fundamentally different, bolder approach, and the job keeps going, as many fresh windows as it takes, until the job is done, so nothing waits on a person who is not there; only the person's own stop still puts a task down (`Options.KeepGoing` wired to `Decider.YoloIsOn`, `boldlyReorientAndKeepGoing`, `TheBoldReorientAsk` in `jobs.go`; 8 September 2026, after the tic-tac-toe run's first task ended on a handoff line and sat waiting for nobody). A job's task is also paced against the job (`pace.go`, at the hook where a round ends in `run.go`): once two of the job's tasks have finished, the median of their running times from `Timing` is the measure, so long as it is at least five minutes; a task that has run twice the median from the start of this run reads one line, once, saying how long against what median, how many done lines are proved, and to prove the open lines with what it holds or write what stands in their way as a failure and close; at three times the harness ends the task and sets it aside the way a twice-stopped task is set aside (`Outcome.PacedOut`, `setAPacedOutTaskAside` in `jobs.go`), so the job goes on to its other tasks and comes back to this one, rather than retrying it until three failures in a row pause the whole job; the paced-out report still reaches the person, a task too slow again on its comeback is put down for the person, and the done check is never bypassed (8 September 2026: a slow task must not eat the job it belongs to). A failure that is neither paced nor a guard stop still counts toward the three-in-a-row pause, which is the safety net for a job that is genuinely broken. A plain task outside a job is never paced (`CostLineAtTimesTheMedian` 2, `CloseAtTimesTheMedian` 3, `LeastMedianToPaceBy` five minutes). The store also keeps, in the same state snapshots the log holds, when the job was made, when each task was first handed out and when it finished, and when the job closed; `Timing` serves them, so the task's report ends with what it took, the job's closing line with what the job took, and the screen draws the running times from the moments the serve sends (`internal/job/timing.go`, `jobtiming.go`). One scheduled job ships with the agent: the nightly self-check at four in the morning (`internal/replay/nightly.go`), which asks the memory twenty questions, dry-runs every skill, and sends the person one line; it waits for a running task to end before it fires. A skill with nothing to replay, one that is its SKILL.md alone or one whose steps take an argument its test file does not give, is a note on that line and not a failure, because the shipped browser and qa skills were counted broken every night until 8 September 2026 (`contract.ErrNothingToDryRun`, `internal/skill/run.go`).
...
| Machine | `shell` | Run a command in the sandbox; `serve` starts a server and answers when its port listens, and the server is not on the command timeout, in the table or in the command handed to the sandbox, since run 25's servers died ten minutes after they started and run 28's an hour after (8 September 2026); `check` asks whether a port answers; a command past ten seconds returns an id to poll, tail or kill |
| Web | `web` | Search the web or fetch a public page as text |
| Browser | `browser_open`, `browser_read`, `browser_screenshot`, `browser_resize`, `browser_click`, `browser_type`, `browser_act`, `browser_login`, `browser_handoff` | Drive the Chrome window on the screen: open a page, read it as an outline of elements with references, take a picture, resize, click by reference or at a point, type by reference, do one action and check it worked, log in from the vault, hand the window to the person. A click at a point, `x` and `y` in whole CSS pixels from the top left of the viewport in place of an element, is for a canvas or anything else the outline does not list, and a call naming both or neither is refused in one sentence (7 September 2026). A `browser_act` step that names no method is read by what it carries: an element or a point is a click, text is a type, a key is a press (8 September 2026). An action's result says "not what was expected" only when the page did not change at all or ran into a wall; when it changed in a way the model's words did not name, the result says what changed and passes no verdict. It carries the whole outline again only when an element was added and the refs moved, and otherwise ends with the page's errors and "outline unchanged; read the page for it". A screenshot that is the same as the last one at its size is said to be the same, with no picture and no new file (8 September 2026) A same picture says whether the page drew frames in the quarter second before it, "the page is alive and nothing on it moved" or "its loop has stopped or the tab is hidden", and a new picture says how many, so a still scene is never taken for a broken camera; and a `browser_read` whose intent says it evaluates something on the page yet carries no ask is refused with an example of the ask to write (8 September 2026). A click at a point that did not do what was expected says what was under the point, the listed element as the outline names it or that nothing listed is there, since run 28 clicked stale coordinates a hundred times without learning what they hit (8 September 2026). When a click changes the element it hit in a way the outline cannot show, such as a game cell filled with an aria-hidden mark and a data attribute while its accessible name stays the same, the result says what the clicked element became, its state such as `button "Cell 3", disabled, data-value="O"`, but only when the outline is otherwise unchanged, so a move on a page whose state lives off the accessibility tree can still be confirmed instead of clicked again; the worker skips this read while a dialog is open, because a page with an open dialog cannot be evaluated until it is answered (8 September 2026) |
