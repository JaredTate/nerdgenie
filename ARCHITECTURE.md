...
**Built and not yet wired.** Nothing outside this package constructs `Recorder`, `Replayer`, `Checker`, or calls `InstallQASkill`: `cmd/coeus` and `internal/loop` do not import it, there is no `/replay` command, and no path installs the shipped `qa` skill into a user's skills folder. The recording, the replay, the self-heal and the visual check are proved by this package's own tests against the fake browser worker of `internal/testkit`, and by its integration test on a real disk, and by nothing else; until the wiring lands, the feature is reachable only from Go. For the record, this package's tests were written alongside its code rather than before it — `9a9384b`, `ef16894`, `63ce7f8` and `90d097d` each land code and tests in one commit — which is why the gate review found a refused step being run by the self-heal, a page steering it, and five bounds pinned by nothing; the tests written since for those findings are failing-test-first, and the commits show it.
