...
`make release` is `scripts/release/build.sh`, and it writes `dist/`: one `coeus-<version>-<architecture>.tar.gz` for `linux/amd64` and `linux/arm64`, a flat `SHA256SUMS` written last that never lists itself, and a `manifest.json` holding the version, the UTC date, the Node version, the architectures, and each archive's checksum, which is what the updater of brief 6.3 reads. The version comes from `git describe` unless `VERSION` says otherwise. Inside an archive are five things: `coeus`, a `VERSION` file, `node/bin/node`, and `workers/browser/` and `workers/desktop/`, each with its bundle at `main.js` beside the dependencies it needs at run time. That is the same shape `bin/workers/<name>/main.js` has in a checkout, so one path finds a worker in a development build and in an install. `scripts/release/workers.sh` builds the two bundles for both `make build` and `make release`, skips a bundle already newer than everything it is built from, and mirrors it into `bin/workers/` with its `node_modules` linked rather than copied.
