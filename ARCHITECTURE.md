...
**The named numbers.** `MaxCheckpoints` could be set to seven with the package still green, which is finding 23; the other five went red when they were moved, which is the standard the gate review measured against. `bounds_test.go` now writes all six as the literals they are meant to be, each with the reason it is that number.
