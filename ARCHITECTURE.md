...
Around any of them, `provider.WithRetries` tries three times in all, waiting one second and then two with up to a quarter added, honouring `Retry-After` in both its forms, and never retrying a request that was simply too long. `provider.NewChain` tries the models in order and moves on when one is out of tries, but hands `contract.ErrContextOverflow` straight back, because only the context builder can fix a prompt that does not fit; `Chain.AnsweredBy` says which model answered, because `contract.Reply` has no room for it. Hermes' two local-server rules are kept: a base address on this machine is probed once at `/props`, and a server that answers the way llama-server does is sent `chat_template_kwargs: {"enable_thinking": false}` and has a smaller reported window preferred over the configured one.

Every wait in the package is measured on `contract.Clock`, including the sixty-second watch that gives up on a stream which has gone quiet, so no test ever waits on the real clock. Wave 0 ships no real implementation of `contract.Clock`, so every caller must supply one.
