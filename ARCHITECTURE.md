...
**`/walk`.** `WalkCommand(WalkOptions{Browser, Model, Ask, Skills, Home})` returns the `contract.Command` the orchestrator registers in `serve.go` beside the skills command, and it is the only door into this package from outside. `/walk` on its own names its three forms. `/walk record <name>` writes the page the browser is on down as the first step of a new walk, saved through the skill store with the one site it was recorded on in its permissions block and no triggers, because a procedure nobody has finished writing should not fire from a message; the answer says in plain words that a person's own clicks are not seen — `contract.BrowserWorker` reports no user event, so the rest of the walk is added by hand in `steps.md` or recorded as the agent takes the steps itself. `/walk replay <name>` replays it, with the self-heal and its three guards, so a step the recording never named is put to the user before it is taken. `/walk check <name>` walks it and photographs every step into `~/.coeus/checks/<name>`, one folder per walk, replacing the pictures of the last check of that walk. The preview goes to the screen the command was typed on when there is one, so the person who asked for the walk is the person asked about it, and every refusal names the rule it broke: no browser, no home, no name, a skill that is not a walk, a name no folder can have.
