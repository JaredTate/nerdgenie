...
│                           How to write an ask, and six worked examples.
...
**The event log is the truth.** `internal/log` keeps one table of events, each with a sequence number that only grows, a time, the task it belongs to, its kind, and its own fields as JSON. The kinds are `message`, `tool call`, `tool result`, `permission decision`, `file change`, `record change`, `checkpoint`, and `reply`. Nothing is ever changed or removed. The full text of every tool result lives here, which is what lets a record hold one line per result and still lose nothing: `read r7` fetches the text back.
...
|---|---|---|
| Files | `read`, `write`, `edit`, `search` | Read a file with line numbers, a folder, one section of a Markdown file by heading, or a past result by its id; write or edit a file inside the allowed folders, the edit finding its span even when the model's copy is not quite the file's; find files by name or lines by pattern |
...
| Browser | `browser_open`, `browser_read`, `browser_screenshot`, `browser_resize`, `browser_click`, `browser_type`, `browser_act`, `browser_login`, `browser_handoff` | Drive the Chrome window on the screen: open a page, read it as an outline of elements with references, take a picture, resize, click and type by reference, do one action and check it worked, log in from the vault, hand the window to the person |
...
- `PROMPT_TEMPLATE_GUIDE.md` and the `EX_PROMPT_*` files: how to write an ask the harness can lift, with six worked examples.
