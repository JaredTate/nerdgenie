...
|---|---|---|
| Files | `read`, `write`, `edit`, `search` | Read a file with line numbers, a folder, one section of a Markdown file by heading, or a past result by its id; write or edit a file inside the allowed folders, the edit finding its span even when the model's copy is not quite the file's and answering with the changed lines and three of context each side, numbered as read numbers them, so the file is not read again; find files by name or lines by pattern |
