...
**Four gaps to report.** `contract.MemoryCaps` has a limit for `MEMORY.md` and one for `USER.md` and none for `SOUL.md`, although the design says all three persona files have one; `context.SoulBytes` stands in at four thousand bytes until the contract carries it. The instruction text of design section 5 is 648 words where the design's own sentence claims it is under five hundred, and its length is now felt: the fixture's own check at a three-thousand-token window has about thirty tokens of room left, so the next paragraph added to it will not fit on a small model; the constant is the design's text unchanged and `MaxInstructionWords` holds the line at what it actually is, so that nothing can be added without a decision. And the boundary is made in `context.New`, which `serve.go` calls once for the life of the daemon, so a boundary the instruction text calls fresh for each task is in fact fresh for each run of the program; moving the builder behind a per-task constructor is the loop's change to make, not this package's. And the layout of the messages was settled by measurement rather than by argument, so the numbers are worth keeping: with the whole record first, as the design's layer table originally had it, two rounds that follow each other shared 6,254 bytes of 18,073, because the result list grew in the middle of the prompt and dragged the whole conversation under it. Putting the conversation first instead would have shared 15,464. Taking only the result list out of the record and putting it at the tail with the budget line shares 15,979 of 17,956 while leaving the record where the model attends to it best, which is why that is what it does.
