...
Below the cache line, in the messages, everything is put in the order of how often it changes, because a provider's prompt cache keeps whatever the last call and this one share from the first byte and stops at the first byte that differs. So: the record's work and lessons, which only grow; the pinned evidence; the recent messages and results; the memory hint; and last of all the record's header, the two lines carrying the budget left and what the last call cost, which are written anew on every single call. The record is cut in three on the `## Goal` and `## Work` headings its own printer always writes: the goal and the rules go above the cache line, the work and the lessons into the first message below it, and the header into the last message before the model speaks.

Where each cache boundary falls, in one line each. **A** ends the instruction text and the persona: they change only when the design changes or the user edits a persona file. **B** ends the tool list: it changes only when the tool set does. **C** ends the record's goal and rules, which change when the user corrects the work. Everything after C is a message, and the messages run from what changes least to what changes most, ending at the budget and cost lines.
