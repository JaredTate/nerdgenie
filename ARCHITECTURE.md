...
**The project folder.** Where names the folder the work lives in, the first backticked path with a slash, and the job keeps it as the situation line "project folder: ..." (`projectfolder.go`). Every task of the job lists that folder in its orientation, reads its documents from it, writes into it, looks for an `exists` check there, and runs every check command inside it. A task with no job works in the home's configured work folder.

**The three project documents.** A folder may carry three files the harness reads for the model and keeps true. `AGENTS.md`, the project's rules and how to run and test it, rides under the job summary on every call of a task in that folder, cut at sixty lines (`front.go`, `internal/context/standingorder.go`). `ARCHITECTURE.md` is never read whole: the orientation names its sections, `read ARCHITECTURE.md <heading>` returns one, and the review's fifth question at a task's end asks which section the task changed and what it should say now, and writes the answer under that heading, dated (`archpage.go`). `REPO_MAP.md` is never read whole either: its roots legend rides in the orientation, and a finished job writes a generated map of the folder beside `AGENTS.md`, which it also writes when there is none (`mapfile.go`, `agentsfile.go`). A file the person wrote is never overwritten.
