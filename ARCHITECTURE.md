...
**`cmd/coeus/firstturn.go` is a stand-in and is meant to be deleted.** Until `internal/loop` lands, `firstTurn.StartTask` is what the router calls for a message that is not a command: it writes the message to the event log, builds a `contract.Request` whose three system blocks are the design's own harness rules from section 5 of `docs/COEUS_PLAN.md`, the three persona files, and the one text form of a tool call, adds the conversation so far, capped at twenty messages and thirty-two kilobytes, makes one model call inside the turn's time budget with no tools, writes the reply to the log, and sends it back through the channel the message came from. The swap is one line in `openTheFront`: `loop.New(...).StartTask` in place of `running.turn.StartTask`. `noSkillsYet` in `serve.go` is the same kind of stand-in for `internal/skill`, and goes the same way in wave 4.
