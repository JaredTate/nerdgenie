...
- `make release` — binaries for `linux/amd64` and `linux/arm64`, the worker bundles, a checksum file, and a manifest, into `dist/`. Until wave 6 builds it, the target says so and exits 1 on purpose.
