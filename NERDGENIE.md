...
| Says when a page draws what its markup calls hidden | ARCHITECTURE.md, the browser worker | 5.1, 5.3 | `marks an element that its markup hides but a style rule draws` in `worker/browser/test/snapshot.test.ts`; `TestAnElementTheMarkupHidesButAStyleRuleDrawsIsSaidSo` in `internal/tool/browserread` |
