...
| Keeps a server started with the shell's serve action running past the command timeout, so a page served for a job is still there when the next task's checks open it | §5 | `TestAServedCommandIsNotOnTheCommandTimeout` in `internal/tool/shell` |
