...
The harness watches for a model going round in circles, and it has a ladder for it. It counts rounds in which nothing measurable moved: no test went green, no step or done line was marked, no page changed under an action, no new file was written, and nothing new was read. At ten such rounds the model reads one line saying so. At twenty, the harness cuts the stalled rounds out of the conversation and keeps everything before them, so the model stops seeing the pattern that was feeding itself; the stall goes into the record as a failure. It also asks for a rethink: one call with the tools off, over the record and the thing the model kept asking for, that asks what it was trying to learn, what the rounds showed, and which one call would tell two causes apart. The answer goes into the record, the repeated call is closed for ten rounds, and a fresh window opens on the answer. The count starts again. After three such cuts, a fourth stall stops the task and tells you. A call the model repeats word for word is refused on the third try and earns the same rethink on the fourth. When the conversation reaches its message cap, the harness opens a fresh window the same way, with the newest results in full and the ask last, instead of dropping half the conversation. Inside a job, a task the harness stopped this way is picked up once by the job itself; if it stops the same way again, the job sets it aside and goes on with its next task, and comes back to it once, before its last task, so twelve tasks that need nothing from a stuck one no longer wait on it; a task set aside that stalls again waits for you. A task you stopped is never picked up behind your back. A job also weighs each task against its neighbours: once two of its tasks have finished, a task that has run twice their median time reads one line saying so and asking it to prove what it can or write what stands in the way and close, and at three times the harness sets it aside the way it sets a stuck task aside, so the job goes on to its other tasks and comes back to this one, rather than retrying it until three failures in a row pause the whole job; a task too slow again on its comeback waits for you. When you run in yolo, though, the agent is working on its own with nobody to wait for, so it never waits at all. A task that would have waited for you — a stuck task on its third stall, a slow one on its comeback, or one that stopped to ask a question — is handed back to the model on a fresh window with a blunt nudge to break its trance: it names what the model already tried and none of it worked, tells it to drop all of that, look with completely fresh eyes, and try a bold, fundamentally different approach. The job keeps going that way, as many fresh windows as it takes, until it is done; the only stop it still honors is one you typed yourself.
...
| Picks a task its own guard stopped up once itself, on a fresh window, before a job sets it aside; never a task the person stopped | ARCHITECTURE.md, the turn loop and the jobs | `TestAJobPicksAGuardStoppedTaskUpOnceItselfOnAFreshWindow` and `TestAPersonsStopIsNeverPickedUpByTheJobItself` in `internal/loop`; `TestAJobLetsATaskBePickedUpOnceAndRemembersItAcrossARestart` in `internal/job` |
...
| Says, when a picture is the same as the last, whether the page is drawing, so a still scene is never taken for a broken camera | §5 | `TestASamePictureOfAPageThatDrawsSaysNothingMoved`, `TestASamePictureOfAPageThatDrawsNoFramesSaysItsLoopStopped` and `TestANewPictureSaysHowManyFramesThePageDrew` in `internal/tool/browsershot`; `TestAnIntentThatEvaluatesWithNoAskIsRefusedWithAnExample` and `TestAnIntentThatOnlyReadsThePageNeedsNoAsk` in `internal/tool/browserread`; `a screenshot of a page that keeps drawing reports the frames it drew`, `a screenshot of a page that drew once reports no frames` and `a screenshot brings the window to the front first` in `worker/browser/test/frames.test.ts` |
...
| Sets a task aside after its second guard stop and goes on with the job, coming back to it once before the last task | §4, §6 | `TestASecondGuardStopSetsTheTaskAsideAndTheJobGoesOn` and `TestATaskSetAsideTwiceWaitsForThePerson` in `internal/loop`; `TestASecondGuardStopDefersTheTaskAndNextTaskPassesOverIt`, `TestADeferredTaskIsTakenAgainWhenOnlyDeferredTasksRemain` and `TestATaskDeferredTwiceIsNotDeferredAgain` in `internal/job`; `TestTheFakeJobDefersATaskOnceAndPassesOverIt` and `TestTheJobCheckCatchesAStoreThatBreaksTheDeferPromise` in `internal/testkit`: on the night of 7 September 2026 the flight simulator's sky task waited nine hours for a person, three times, while twelve tasks that needed nothing from it sat untouched |
| Tells a task its cost against the job's median at twice it, and at three times sets it aside so the job goes on to its other tasks and comes back to it, instead of retrying it until three failures pause the job | §4, §6 | `TestATaskAtTwiceTheJobsMedianReadsItsCostLineOnce`, `TestATaskAtThriceTheJobsMedianIsSetAsideSoTheJobGoesOn`, `TestAJobWithFewerThanTwoFinishedTasksPacesNothing`, `TestAJobWhoseTasksFinishInsideTheFloorPacesNothing` and `TestAPlainTaskOutsideAJobIsNeverPaced` in `internal/loop`: the sky task ran 443 rounds and ten hours where its neighbours took 84 to 173 rounds, almost all of it on one done line |
| Says what the clicked element became after a click, its state such as `button "Cell 3", disabled, data-value="O"`, when the outline shows nothing changed, so a move that fills a cell whose mark is off the accessibility tree can still be confirmed instead of clicked again | §5 | `aimed-state.test.ts` in `worker/browser`; `TestAChangeSaysWhatTheClickedElementBecameWhenTheOutlineIsUnchanged` in `internal/tool/browserread` |
