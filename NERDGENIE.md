...
| Refuses a task with more than eight plan steps and sends the model to the `job` tool | §4, §5 | 1.2, and the `task` tool in 2.5 | Eight steps are kept and nine refused, with the refusal naming the `job` tool, in `internal/record/plancap_test.go`; the rules sentence is checked against the design |
