...
| Lists the loops, file and line, in the scripts the task changed when the browser reports a page whose own script never yields | §5, §6 | 3.1 | `TestAHungPageListsTheLoopsInTheScriptsTheTaskChanged` and `TestAnOrdinaryBrowserResultListsNoLoops` in `internal/loop/hangs_test.go` |
