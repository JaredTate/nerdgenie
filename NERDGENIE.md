...
|---|---|---|
| Files | `read`, `write`, `edit`, `search` | Read a file, a folder, one section of a Markdown file by its heading, or a past result by its id. Write or edit a file inside the allowed folders. Find files or lines |
...
| Drops the oldest half of the conversation at once when its cap is reached, so the front of the prompt holds still between drops | §7 | `TestTheOldestHalfOfTheMessagesLeavesAtOnceWhenTheCapIsReached` in `internal/loop/window_test.go` |
...
| Works on the local Qwen, Opus 4.8, and GPT-5.5 | §4 | The live forty-round task from wave 2 onward |
