...
| Tells a task its cost against the job's median at twice it and closes it honestly at three times | §4, §6 | `TestATaskAtTwiceTheJobsMedianReadsItsCostLineOnce`, `TestATaskAtThriceTheJobsMedianEndsFailedWithItsOpenLinesOnTheReport`, `TestAJobWithFewerThanTwoFinishedTasksPacesNothing`, `TestAJobWhoseTasksFinishInsideTheFloorPacesNothing` and `TestAPlainTaskOutsideAJobIsNeverPaced` in `internal/loop`: the sky task ran 443 rounds and ten hours where its neighbours took 84 to 173 rounds, almost all of it on one done line |
