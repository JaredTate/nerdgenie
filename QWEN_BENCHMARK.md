...
Coeus, opencode, Hermes and OpenClaw on the same tic-tac-toe task, each driving
...
setup it ships with and no cap on anyone, Coeus finished the task green both
...
reasoning field). Coeus sends the field set to false; the other three send
...
- **The daemons' cache.** Round 1 began with both daemons restarted cold for
  Coeus and Hermes; opencode and OpenClaw then ran on the same daemons without
...
- **One harness per card.** Round 1: Coeus and opencode on card A, Hermes and
  OpenClaw on card B. Round 2 on card A alone, one after another, while the
  user tests Coeus by hand on card B. A run never shares a card.
- **No cap of any kind.** Coeus's own task budget raised out of reach; each
...
- **The same referee for tokens.** Every harness talks to the same daemon, and
  ...
  cross-check.
- **The same clock**: launch to exit. Coeus's driver leaves a short quiet
...
|---|---|---|---|---|---|---|---|---|---|---|
| **Coeus** | **yes** | 4 min 55 s (task in to answer out 4 min 15 s) | 24 | 253,214 | 82,577 | 15,858 | 5,821 | 6/6 | 4/4 | 6 of 6 |
...
1. **Everyone builds a working game; only Coeus stops when it is done.** All
   ...
   everyone: Coeus fixed it and stopped, in 24 calls the first time and 55
   the second; the other three fixed the game but never got their own draw
   test green, and kept trying for hundreds of calls.
2. **Context is the whole story.** Coeus's biggest prompt was 15,858 tokens in
   ...
   sees; Coeus never shows it the attempts, only one line per failure with
...
3. **Repetition is refused, not detected.** Hermes's loop was the same two
   ...
   its probes varied by a character. Coeus refuses the second identical call
   outright and tells the model so.
4. **The card's real work.** "Read afresh" is what the daemon had to process:
   Coeus 82,577 and 223,330 tokens; Hermes 65,320; opencode 25,613; OpenClaw
...
5. **Coeus asked for a yes.** Its permission function stopped six times in
...
say what tuning buys a transcript harness on a small model. Coeus was never
tuned.
