...
- Built at 05:20 on 6 September, from the tenth nightly run: a reply cut off at the output cap (8,192 tokens, two minutes of generation) was read as an unreadable call and answered with the three options, and after three such rounds the frontend task took "answer the user" and closed done with no game file written; the play-test task did the same. The loop now reads the finish before the parse, keeps none of a cut-off reply, and tells the model it was cut off after that many tokens and to write a long file in parts of at most three hundred lines, the first by write and the rest by edit (`loop/cutoff.go`). Behind it, an answer on a record with no done list and a plan step still open is sent back to the plan, naming the open steps, three times and then failed (`loop/openplan.go`). Still to measure: whether the local model splits a file on the first telling, and whether 300 lines is the right part.
...
- Built at 05:56 on 6 September, from the eleventh nightly run: the harness read none of the game job's ten Vitest runs as a test run, because the test-state reader knew Node's own runner, Jest, pytest and Go's and not Vitest's "Tests  16 failed | 56 passed (72)". So no tests-after-a-change rerun fired, a run that went from 36 failing to 16 was "finished with exit code 1" both times, the meter saw twenty rounds of real implementation as no progress, and it cleared the conversation in the middle of the hazards task. The reader now knows Vitest, files that could not load included. The run was stopped at 05:54 on its third task so that the twelfth could start on the binary with every fix of the night.

- **Found at 06:30 on 6 September, and the owner's to decide: the daemon's checkpoint spacing costs more than half of every round.** The twelfth run's engine task showed the cached count sitting flat for stretches of fourteen rounds (21.3k, then 32.0k, then 40.6k) while the prompt grew, and the daemon's own log showed why: its prompt processing per request grew round by round (5,893, 6,456, 7,268, 8,512, 9,052 tokens) and then dropped (1,405) and grew again. The model is a hybrid, `qwen35` with SSM layers and full attention every few layers, so its state cannot be rolled back to an arbitrary token: llama-server keeps context checkpoints and, when a prompt diverges from the last one (which every round's does, at the start of the record's tail), restores the newest checkpoint before the divergence and re-processes everything after it. Checkpoints are spaced `--checkpoint-min-step` apart, 8,192 tokens by default, so each round re-processes on average half of that plus the tail. Measured over seventy requests: 888 seconds of prompt processing (64 percent of the model's time) for a mean of 4,875 tokens a request, against 489 seconds of generation. Every harness-side cut to the tail is worth a few hundred tokens; this is worth about four thousand a round. The one-line change is on the daemon's start line, which is never overridden from here: `--checkpoint-min-step 1024` (checkpoints every thousand tokens, the newest thirty-two kept, which covers the last thirty-two thousand tokens where every divergence lands) would bring the re-processing to about the tail and the new round, some 1,500 tokens, cutting a twenty-second round to about twelve. The cost is the memory of thirty-two SSM-state checkpoints on a card that has been out of memory once this week, which is why it is the owner's call; `--checkpoint-min-step 2048` is the half-way step.
...
- **Built at 09:20 on 6 September, with the owner's go-ahead: the model's eyes, and the checkpoint spacing.** The projector file for this exact model was in the same Hugging Face repository as the quant; `~/llm/igo.sh` now carries `--mmproj` and `--checkpoint-min-step 1024 --ctx-checkpoints 16`, both env-overridable, with the old script kept beside it. The daemon reports vision on, a 1024 by 768 screenshot costs 799 prompt tokens, and through the harness the model opened the game, took a `browser_screenshot`, and described it down to the next piece and the four HUD numbers. Idea 12 is built: a picture from the browser, a picture file, or the desktop rides with its result to a seeing model, the newest two kept, and a model with no eyes is told the picture is not shown. Still to measure on the fourteenth run: the uncached tokens a round under the new spacing, and what the play-test does with eyes.
