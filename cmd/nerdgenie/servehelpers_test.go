...
func TestWhichBinaryIsServingNamesTheProgram(t *testing.T) {
	...
	for _, want := range []string{"nerdgenie", "commit", "serving from"} {
...
// configuration and a database from a newer Nerd Genie stop the service, while
...
func TestExitCodeForStopsTheServiceOnlyForWhatARestartCannotFix(t *testing.T) {
	...
	if got := exitCodeFor(update.ErrDatabaseFromANewerNerdGenie); got != contract.ExitBadConfiguration {
		t.Errorf("a database from a newer Nerd Genie exited with %d, want %d", got, contract.ExitBadConfiguration)
	}
