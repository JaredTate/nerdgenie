...
3. A task has no budget unless the user sets one. Coeus puts no cap on its own work: `rounds_per_task`, `time_per_task`, and `time_per_turn` are off by default, and a user who wants a budget sets one in `config.toml`; a skill can set its own. When a budget that was set runs out, the model gets one last call with tools turned off. In that call it says what it did and what is left, and the user gets that report.
