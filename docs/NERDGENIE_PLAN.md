...
>
> **The project's documents.** A folder may carry NERDGENIE.md (its rules), ARCHITECTURE.md (a section per part) and REPO_MAP.md (every file's functions). The harness writes all three at a task's end; never plan or edit them. Look a file or function up in the map before you list, search or read: `read REPO_MAP.md` lists the files, `read REPO_MAP.md <path>` gives a file's functions; ls, find, grep and search are for what it lacks. Read a section, never the whole: `read ARCHITECTURE.md <heading>`, `read ask <heading>`. Asked at a task's end what a section should say, answer with the heading first and one paragraph under it.
>
