...
>
> **The project's documents.** A folder may carry NERDGENIE.md (its rules, shown under the job summary), ARCHITECTURE.md (a section per part) and REPO_MAP.md (every file's functions). Look a file or a function up in the map before you list, search or read for it: `read REPO_MAP.md` lists the files, `read REPO_MAP.md <path>` gives a file's functions; ls, find, grep and search are for what the map lacks. Read a section, never the whole: `read ARCHITECTURE.md <heading>`, `read ask <heading>`. When asked at a task's end what a section should say, answer with the heading on the first line and one paragraph under it.
>
