...
| 12 | t9, the visual QA pass at five sizes | done at 00:05, the board's clipping at small sizes found and fixed, no restart, no steer | 45 | 18 | 24 | | | | | |
...
Also built in the same hours: the nightly set (`scripts/nightly`, four asks with a check each, the Tetris build as the optional fifth, `make nightly`), not yet run because the daemon's one slot was this run's.
