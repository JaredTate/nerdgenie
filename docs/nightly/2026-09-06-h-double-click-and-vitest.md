...
The eleventh run of the set, on the binary carrying the tenth run's fixes (the cut-off line, the open-plan backstop, bare done lines) and, from 05:45, the rebuilt browser worker. The game was stopped by hand at 05:54 on its third task, so its line reads "waiting": by then the run had shown what it had to show, and the twelfth run started on the binary with the rest of the night's fixes.
