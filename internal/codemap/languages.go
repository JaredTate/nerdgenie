...
var (
	scriptFunction = regexp.MustCompile(`^(?:export\s+)?(?:default\s+)?(?:async\s+)?function\s*\*?\s*([A-Za-z_$][\w$]*)\s*\(([^)]*)\)`)
	scriptArrow    = regexp.MustCompile(`^(?:export\s+)?(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*(?::[^=]+)?=\s*(?:async\s*)?(?:\(([^)]*)\)|[A-Za-z_$][\w$]*)\s*=>`)
	scriptClass    = regexp.MustCompile(`^(?:export\s+)?(?:default\s+)?(?:abstract\s+)?class\s+([A-Za-z_$][\w$]*)`)
	scriptConst    = regexp.MustCompile(`^export\s+const\s+([A-Za-z_$][\w$]*)\s*(?::[^=]+)?=`)
	scriptMethod   = regexp.MustCompile(`^\s{2,}(?:public\s+|private\s+|protected\s+|static\s+|async\s+|readonly\s+)*(?:get\s+|set\s+)?([A-Za-z_$][\w$]*)\s*\(([^)]*)\)\s*(?::\s*[^{]+)?\{`)
	...
	cMethod   = regexp.MustCompile(`^\s{2,}(?:[A-Za-z_][\w:<>,\s\*&]*\s+[\*&]*)?([A-Za-z_]\w*)\s*\(([^;{}]*)\)\s*(?:const\s*)?(?:override\s*)?(?:noexcept\s*)?\{`)
	...
	javaMethod = regexp.MustCompile(`^\s{2,}(?:public|private|protected|static|final|abstract|synchronized|\s)*[\w<>\[\],\s]+\s+([A-Za-z_]\w*)\s*\(([^)]*)\)\s*(?:throws[^{]*)?\{`)
...
var languages = map[string]language{
	...
	".go": {defs: []definer{{goMethod, "method", 1, 2, false}, {goFunc, "func", 1, 2, false}, {goType, "type", 1, 0, false}}, comment: slashComment, test: goTest},
	".py": {defs: []definer{{pythonDef, "func", 1, 2, false}, {pythonMeth, "method", 1, 2, true}, {pythonClass, "class", 1, 0, false}}, comment: pythonComment, test: pythonTest, docstringBelow: true},
	".rs": {defs: []definer{{rustFn, "func", 1, 2, false}, {rustType, "type", 1, 0, false}}, comment: rustComment, test: rustTest},
	".rb": {defs: []definer{{rubyClass, "class", 1, 0, false}, {rubyDef, "func", 1, 2, false}}, comment: hashComment, test: rubyTest},
	".c":  cLanguage, ".h": cLanguage, ".cc": cLanguage, ".cpp": cLanguage, ".cxx": cLanguage, ".hpp": cLanguage, ".hh": cLanguage,
	".java": {defs: []definer{{javaClass, "class", 1, 0, false}, {javaMethod, "method", 1, 2, true}}, comment: slashComment, test: javaTest},
}
