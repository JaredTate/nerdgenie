...
func TestTheWaitsAreTheOnesTheDesignAsksFor(t *testing.T) {
	for _, one := range []struct {
		...
		{"the whole of nerdgenie init", command.SetupWait, 20 * time.Minute},
...
func TestInitResetConfigLeavesAPersonaFileSomebodyWroteAlone(t *testing.T) {
	...
	if string(after) != theWordsAPersonWrote {
		t.Errorf("nerdgenie init --reset-config wrote over what somebody wrote themselves:\n%s", after)
	}
}
...
func TestInitClosesAHomeFolderOtherAccountsCanLookInside(t *testing.T) {
	...
	if err := os.MkdirAll(home.Root, contract.HomeFolderMode); err != nil {
		t.Fatalf("making the home folder before nerdgenie init ran failed: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("nerdgenie init on a home folder that was already there failed: %v", err)
	}
	...
	for _, one := range []struct {
		...
		t.Run(one.what, func(t *testing.T) {
			...
			if one.works && err != nil {
				t.Fatalf("nerdgenie init gave up before the third try at the menu: %v", err)
			}
			if !one.works && err == nil {
				t.Fatalf("nerdgenie init went on asking past the third try at the menu")
			}
		})
	}
}
...
func TestWhatTheServiceManagerPrintedIsCappedBeforeItGoesInAMessage(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("nerdgenie install said nothing when the service manager refused")
	}
