...
var evasions = []struct {
	...
	{"a long prefix that pushes the delete past the cap", strings.Repeat("echo hello; ", 40) + "rm -rf /home/jared/coeus"},
	{"a delete handed to a nested shell", `sh -c "rm -rf ~"`},
	{"a delete handed to a nested login shell", `bash -lc 'rm -rf ~'`},
	{"sudo written as the full path to the program", "/usr/bin/sudo -u root rm -rf /home/jared/coeus"},
	{"a program a command substitution works out while it runs", "$(printf rm) -rf /home/jared/coeus"},
}
...
var programsWrittenAsAFullPath = []struct {
	...
	{"/usr/bin/sudo -u root rm -rf /home/jared/coeus", "sudo rm -rf"},
...
func TestSudoWrittenAsAFullPathIsStillCaughtBySudo(t *testing.T) {
	...
	decision := decide(t, decider, shellRequest(t, "/usr/bin/sudo -u root rm -rf /home/jared/coeus"))
	if decision.Ruling != contract.RulingAsk {
...
var tooLongToRead = []struct {
	...
	{"a readable form longer than the cap", strings.Repeat("echo hello; ", 40) + "rm -rf /home/jared/coeus"},
}
