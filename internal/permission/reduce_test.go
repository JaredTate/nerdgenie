...
var shellReductions = []struct {
	...
	{"git -c user.name=nobody commit -m \"a note\"", "git commit"},
	{"make -C /tmp/build clean", "make clean"},
	{"docker compose -f /tmp/compose.yml up -d", "docker compose up"},
	{"rm -rf /tmp/x", "rm -rf"},
	{"rm -r /tmp/x", "rm -r"},
	{"rm -fr /tmp/x", "rm -fr"},
	{"rm -f -r /tmp/x", "rm -f -r"},
	...
	{"env -i sh -c 'rm -rf /tmp/x'", "sh -c rm -rf"},
	{"nohup rm -rf /tmp/x", "nohup rm -rf"},
	{"timeout 60 rm -rf /tmp/x", "timeout rm -rf"},
	{"timeout -s KILL 60 rm -rf /tmp/x", "timeout rm -rf"},
	{"nice -n 10 make build", "nice make build"},
	{"su -c 'rm -rf /tmp/x'", "su -c rm -rf"},
	{"busybox rm -rf /tmp/x", "busybox rm -rf"},
	{"doas rm -rf /tmp/x", "doas rm -rf"},
	{"echo /tmp/x | xargs rm -rf", "echo | xargs rm -rf"},
