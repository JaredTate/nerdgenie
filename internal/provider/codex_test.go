...
func TestTheCodexProviderReadsAnyOtherIncompleteReasonAsStoppedAndNotesIt(t *testing.T) {
	backend := newCodexBackend(t, codexAnswer{events: codexEvents(
	...
	if lines.count() == 0 || !strings.Contains(lines.all()[0], "content_filter") {
		t.Errorf("the reason the backend gave was not written to the log: %v", lines.all())
	}
}
