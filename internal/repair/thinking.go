...
// withoutThinking returns the pieces of the text that sit outside the think
// blocks, and says whether a block was still open when the text ran out.
//
...
// swallows the rest of the text, because half a thought is still a thought. The
// pieces come back separately rather than joined so that a block taken out of
// the middle of a reply leaves no gap behind, which is the same treatment a
// tool-call envelope gets.
//
...
func withoutThinking(text string) ([]string, bool) {
	...
	for at < len(text) {
		...
		kept = append(kept, text[at:at+opening[0]])
		after := at + opening[1]
	...
	return append(kept, text[at:]), false
}
