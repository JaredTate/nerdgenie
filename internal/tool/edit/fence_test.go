...
func stillHolds(t *testing.T, path string, held string) {
	...
	if string(after) != held {
		t.Errorf("the file outside the root now holds %q, and nothing may edit through the fence", after)
	}
}

func TestAnEditThroughASymbolicLinkOutOfTheRootIsRefused(t *testing.T) {
	...
	if _, err := run(t, tool, map[string]any{"path": link, "old": "hunter2", "new": "changed"}); err == nil {
		t.Errorf("an edit through a symbolic link out of the root was made")
	}
	stillHolds(t, secret, "the password is hunter2\n")
}

func TestAnEditThroughAHardLinkOutOfTheRootIsRefused(t *testing.T) {
	tool, root, _ := newTool(t)
	secret := aSecretOutside(t, root)
	...
	if _, err := run(t, tool, map[string]any{"path": link, "old": "hunter2", "new": "changed"}); err == nil {
		t.Errorf("an edit through a hard link out of the root was made")
	}
	stillHolds(t, secret, "the password is hunter2\n")
}
