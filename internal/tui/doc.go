...
// frame is drawn before the link is made, so the person sees the screen at once,
// and the status strip tells the truth about the link the whole time. The
// drawing this package implements is docs/TUI_DESIGN.md, and every rule in it,
// from the five-hundred-millisecond spinner delay to the masked prompt that
// never echoes what is typed, is a test in this folder.
package tui
