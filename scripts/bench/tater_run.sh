    ...
    "provider": {
        "bench": {
            ...
            "models": {
                model: {
                    "name": model,
                    "limit": {"context": int(context), "output": 32000},
                },
            },
        },
    },
...
PYTHON
    ...
    LAUNCH_CMD=("$OPENCODE_BIN" run -m "bench/$MODEL" --format json "$(cat "$WORK/task.txt")")
    ;;
...
open(config_path, "w").write(
    ...
    "  shared_metrics:\n"
    "    enabled: false\n"
    % (model, base_url, context, base_url, model, context)
)
PYTHON
    LAUNCH_ENV=("HERMES_HOME=$HOMEDIR/hermes")
    LAUNCH_CMD=(hermes chat -q "$(cat "$WORK/task.txt")" --provider bench -m "$MODEL" --yolo --in "$WORK")
    ;;
...
STARTED="$(date +%s)"
if [ -n "$MINUTES" ]; then
  env -C "$WORK" "${LAUNCH_ENV[@]}" timeout --kill-after=30 "${MINUTES}m" "${LAUNCH_CMD[@]}" \
    > "$RUN/harness.out" 2>&1 < /dev/null &
else
  env -C "$WORK" "${LAUNCH_ENV[@]}" "${LAUNCH_CMD[@]}" > "$RUN/harness.out" 2>&1 < /dev/null &
fi
HARNESS_PID=$!
