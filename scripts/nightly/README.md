...
Four real asks, run one after another against the local model on a home of their own, each followed by a check that says pass or fail without a person, each measured by `scripts/runreport`. It is the number before and after for every change to the harness. The Tetris build is the optional fifth, an hour or more on its own.
    ...
    make build
    scripts/nightly/run.sh ~/work/ng-nightly/home            # the four
    scripts/nightly/run.sh ~/work/ng-nightly/home --with-tetris

The home is any initialised Nerd Genie home (`nerdgenie init` once), with the work folder beside it as its sandbox root, which is where each ask's fresh folder goes: `nerdgenie init -work-folder ~/work/ng-nightly/work` for the home above. The runner does not serve from that home: every run copies its config, persona, skills and vault key into a fresh home beside the work folder (`homes/<date-time>`, never under a sandbox root, which the serve refuses) and serves from there, so nothing one run leaves running or put down reaches the next. An ask that becomes a job is waited for until the job stops running, within the ask's timeout, and measured over every task from the ask's own task on. Never run the set beside a live run: the local daemon has one slot. The results go to `docs/nightly/<date>.md`, one row per ask: how the task ended, the check, the rounds, the minutes, the seconds a round, the cache share, the output tokens, the rounds that only wrote the record, and the marks made from the reply's first line. Copy the row that matters into `docs/PROGRESS.md` when a change is being judged.
