...
The home is any initialised Nerd Genie home (`nerdgenie init` once). Never run the set beside a live run: the local daemon has one slot. The results go to `docs/nightly/<date>.md`, one row per ask: how the task ended, the check, the rounds, the minutes, the seconds a round, the cache share and the output tokens. Copy the row that matters into `docs/PROGRESS.md` when a change is being judged.
