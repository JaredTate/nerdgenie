...
The home is any initialised Nerd Genie home (`nerdgenie init` once), with the work folder beside it as its sandbox root, which is where each ask's fresh folder goes: `nerdgenie init -work-folder /home/jared/work/ng-nightly/work` for the home above. The runner does not serve from that home: every run copies its config, persona, skills and vault key into a fresh home under the run's work folder and serves from there, so nothing one run leaves running or put down reaches the next. An ask that becomes a job is waited for until the job stops running, within the ask's timeout, and measured over every task from the ask's own task on. Never run the set beside a live run: the local daemon has one slot. The results go to `docs/nightly/<date>.md`, one row per ask: how the task ended, the check, the rounds, the minutes, the seconds a round, the cache share, the output tokens, the rounds that only wrote the record, and the marks made from the reply's first line. Copy the row that matters into `docs/PROGRESS.md` when a change is being judged.
