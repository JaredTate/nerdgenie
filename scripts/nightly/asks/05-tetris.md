<!-- EX_PROMPT_3_TETRIS.md: the Tater Tots Tetris ask written as a work order, the
     ...
     the game goes in, such as ~/Desktop.
...
Create a new folder named exactly `Tater Tots Tetrisv1`, so the project lives at `<WORK>/Tater Tots Tetrisv1`, where <WORK> is the folder the runner names. Make it if it is not there. Put all source code, tests, assets, sound effects and supporting files inside it, and nothing anywhere else. Serve the game from that folder on port 8091.
