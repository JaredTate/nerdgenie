...
# watch "coeus update" take the bad one, fail its readiness check, and go back
# to the good one within sixty seconds. It prints the two commands to run.
#
# Usage: scripts/trial/bad-release.sh [folder]   (default /tmp/coeus-releases)
set -eu

folder="${1:-/tmp/coeus-releases}"
...
say "(coeus install, then systemctl --user status coeus.service). Then, in order:"
say ""
say "  coeus update --from $folder/good     # takes $good and stays on it"
say "  coeus update --from $folder/bad      # takes $bad, which never answers /readyz,"
